...
def copy_image_to_script_dir(file_path):
    """Copies an image file to the script directory."""
    script_dir = os.path.dirname(os.path.realpath(__file__))
    destination = os.path.join(script_dir, os.path.basename(file_path))
    shutil.copy(file_path, destination)
    print(f"Copied image: {file_path} to {script_dir}")
        ...
        for file in files:
            ...
            try:
                ...
                if is_image(file_path):
                    copy_image_to_script_dir(file_path)
